...
# this MUST be a directory called "0.0.6" because we grab the "0.0.6" version from the svn repository, it's hardcoded in this script:
BIONET_SRC_DIR=os.path.join(PATH_TO_WORK_DIR, '0.0.6')
print "=====>=====> BIONET_SRC_DIR = " + BIONET_SRC_DIR

# assure that the source directory is cleared, we're going to build cc3d from it.
# the diretory has to be removed but NOT recreated right away, because it'll be downloaded using svn:
if  os.path.isdir(BIONET_SRC_DIR):
...
BIONET_BUILD_DIR_FOR_CMAKE=os.path.join(PATH_TO_WORK_DIR, 'bionet_build_dir')
print "=====>=====> BIONET_BUILD_DIR_FOR_CMAKE = " + BIONET_BUILD_DIR_FOR_CMAKE

# first clean, then create the directory -
if  os.path.isdir(BIONET_BUILD_DIR_FOR_CMAKE):
    print "=====>=====> Build directory (for CMAKE doing BionetSolver) exists... Removing ", BIONET_BUILD_DIR_FOR_CMAKE, " and creating new directory."
...
print "=====>=====> obtaining the latest BionetSolver source code from the online svn repository:"
# cd to the directory we're using to hold all, and grab the latest source code from svn repository:
os.chdir(CUR_DIR)
call('pwd')
call('svn export http://code.compucell3d.org/svn/cc3d/branch/BionetSolver/0.0.6',shell=True)

print "=====>=====> building BionetSolver:"
# cd to the directory holding all the cc3d source code, and compile it:
os.chdir(BIONET_SRC_DIR)
call('pwd')

print "=====>=====> prepare all make files using cmake with a few command-line settings/options:"

call('cmake -DCMAKE_C_COMPILER='+GCC_460+'/usr/local/bin/gcc -DCMAKE_CXX_COMPILER='+GCC_460+'/usr/local/bin/g++ -DCMAKE_INSTALL_PREFIX:PATH='+BIONET_BUILD_DIR_FOR_CMAKE , shell=True)

# the following would build bionetsolver with the gcc compiler provided with Mac OS X 10.6:
# cmake -DCMAKE_INSTALL_PREFIX:PATH=.....build_test_20110718 -DCMAKE_OSX_DEPLOYMENT_TARGET:STRING=10.6 -DCMAKE_OSX_ARCHITECTURES:STRING=x86_64


print "=====>=====> make BionetSolver:"
call('make install -j8',shell=True)
os.chdir(BIONET_BUILD_DIR_FOR_CMAKE)
call('pwd')
call('find . -exec setlabel -s Orange {} \;',shell=True)
...
# reminder from rsync's man page:
#  a trailing / on a source as meaning "copy the contents of this directory" as opposed to "copy the directory by name" :
call('rsync -rlPp '+BIONET_BUILD_DIR_FOR_CMAKE+'/include/  '+BIN_DIR+'/include ',shell=True)
