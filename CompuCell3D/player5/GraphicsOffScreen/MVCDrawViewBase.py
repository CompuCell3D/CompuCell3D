...
class MVCDrawViewBase:
    ...
    def clearDisplay(
            self):  # called whenever user selects a different field to render; beware NOT doing this because, for example, it wouldn't redraw a dynamic scalarbar
    ...
    def drawConField(self, _bsd, fieldType):
        pass

    def drawVectorFieldCellLevel(self, _bsd, fieldType):
        pass

    def drawVectorField(self, _bsd, fieldType):
        pass

    def drawScalarFieldCellLevel(self, _bsd, fieldType):
        pass

    def drawScalarField(self, _bsd, fieldType):
        pass
    ...
    def drawCustomVis(self, bsd, fieldType):
        ...
        if not self.customActors.has_key(visName):
            ...
            for actorName in actorsDict.keys():
                ...
                if actorType[
                   0:3] == 'vtk':  # check, since we now piggyback off the actorsDict to allow a 'customVTKScript' for .dml
    ...
    def checkIfCameraSettingsHaveChanged(self, _cs):
        ...
        if focalPoint[0] != _cs.focalPoint[0] or focalPoint[1] != _cs.focalPoint[1] or focalPoint[2] != _cs.focalPoint[
            2]:
            return True
