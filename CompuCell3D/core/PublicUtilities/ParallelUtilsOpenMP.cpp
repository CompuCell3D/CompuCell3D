...
ParallelUtilsOpenMP::ParallelUtilsOpenMP() : numberOfWorkNodes(0),
                                             //by default this var is set to 0 . user can set manually number of work nodes
                                             threadsPerWorkNode(1)
//by default this var is set to 1 . user can set manually number threads per worknode
{
...
void ParallelUtilsOpenMP::handleEvent(CC3DEvent &_event) {
    if (_event.id == LATTICE_RESIZE) {
        ...
        init(ev.newDim);
        //setDim(ev.newDim);
    } else if (_event.id == CHANGE_NUMBER_OF_WORK_NODES) {
        // I am trying to gradually get rid of VPU's and in this version we set threadsPerWorkNode which means that, by default, number of nodes is equal to number of threads. Users still can change it
...
void ParallelUtilsOpenMP::setNumberOfWorkNodes(unsigned int _num) {
    ...
    if (_num > 0) {
        ...
        omp_set_dynamic(0);
        omp_set_num_threads_u(numberOfWorkNodes);
        calculateFESolverPartition();
        calculatePottsPartition();
    }
}
...
void ParallelUtilsOpenMP::setNumberOfWorkNodesAuto(unsigned int _requested_num_work_nodes) {
    if (_requested_num_work_nodes < 0) {
        _requested_num_work_nodes = omp_get_num_threads_u();
        // in case we get 0
    ...
    omp_set_dynamic(0);
    omp_set_num_threads_u(_requested_num_work_nodes);
}
...
unsigned int ParallelUtilsOpenMP::getNumberOfWorkNodesFESolver() {
    return static_cast<unsigned int>(feSolverPartitionVec.size());
}
...
unsigned int ParallelUtilsOpenMP::getMaxNumberOfWorkNodesFESolver() {
    ...
    return (numberOfProcessors >= numberOfWorkNodes
                ? threadsPerWorkNode * numberOfProcessors
                : threadsPerWorkNode * numberOfWorkNodes);
}
...
void ParallelUtilsOpenMP::prepareParallelRegionFESolvers(bool _useBoxWatcher) {
    omp_set_dynamic(0);
    if (_useBoxWatcher) {
        omp_set_num_threads_u(getNumberOfWorkNodesFESolverWithBoxWatcher());
    } else {
        omp_set_num_threads_u(getNumberOfWorkNodesFESolver());
    }
}


unsigned int ParallelUtilsOpenMP::getNumberOfWorkNodesFESolverWithBoxWatcher() {
    return static_cast<unsigned int>(feSolverPartitionWithBoxWatcherVec.size());
}
...
void ParallelUtilsOpenMP::calculateFESolverPartitionWithBoxWatcher(const Dim3D &_boxMin, const Dim3D &_boxMax) {
    ...
    unsigned int numProcs = (numberOfWorkNodes ? numberOfWorkNodes : 1);
    //by default we will use 1 thread. Users have to explicitely request more processors

    unsigned int optimalNumberOfThreads = (numProcs == 1 ? 1 : threadsPerWorkNode * numProcs);

    const int zExtent =
            static_cast<int>(_boxMax.z) - static_cast<int>(_boxMin.z);

    const int yExtent =
            static_cast<int>(_boxMax.y) - static_cast<int>(_boxMin.y);
    ...
    if (zExtent == 1 || zExtent < static_cast<int>(optimalNumberOfThreads)) {
        //2D case or quasi 2D case, we will divide thread workloads according fieldDim.y
        if (yExtent > static_cast<int>(optimalNumberOfThreads)) {
            // fieldDim.y is large enough so that it can be divided
            unsigned int threadNum = optimalNumberOfThreads;

            unsigned int wL = yExtentU / (threadNum); //base workload for single processor
            unsigned int extraWL = (yExtentU) % threadNum;
            //extra workload - will be distributed evenly among first processes
            ...
            for (unsigned int i = 0; i < threadNum; ++i) {
                const auto wl_y = static_cast<cc3d_dim_t>(wL);
            ...
            unsigned int threadNum = (yExtentU);
            // number of threads in this case is the same as the box dimension in the y direction
            ...
            for (unsigned int i = 0; i < threadNum; ++i) {
                const auto wl_z = static_cast<cc3d_dim_t>(wL);
                ...
                maxDim = Dim3D(_boxMax.x, minDim.y + wl_z, _boxMax.z);
                feSolverPartitionWithBoxWatcherVec.push_back(make_pair(minDim, maxDim));
                minDim.y = maxDim.y;
            }
        }
    ...
    if (zExtent >= static_cast<int>(optimalNumberOfThreads)) {
        //z dimension is large enough so that we can divide lattice using optimal division scheme


        unsigned int threadNum = optimalNumberOfThreads;
        unsigned int wL = (zExtentU) / (threadNum); //base workload for single processor
        unsigned int extraWL = (zExtentU) % threadNum;
        //extra workload - will be distributed evenly among first processes
        ...
        for (unsigned int i = 0; i < threadNum; ++i) {
            ...
            maxDim = Dim3D(_boxMax.x, _boxMax.y, minDim.z + wl_z);
            if (extraWL > 0) {
                maxDim.z++;
                extraWL--;
            }
...
void ParallelUtilsOpenMP::calculateFESolverPartition() {
    ...
    if (static_cast<unsigned int>(fieldDim.z) == 1 || static_cast<unsigned int>(fieldDim.z) < optimalNumberOfThreads) {
        //2D case or quasi 2D case, we will divide thread workloads according fieldDim.y
        if (static_cast<unsigned int>(fieldDim.y) > optimalNumberOfThreads) {
            // fieldDim.y is large enough so that it can be divided
            unsigned int threadNum = optimalNumberOfThreads;
            // unsigned int wL=fieldDim.y/(threadNum);
            // unsigned int extraWL=fieldDim.y % threadNum ;

            const cc3d_dim_t yExtent = fieldDim.y;

            //base workload for single processor
            const cc3d_dim_t wL =
                    yExtent / static_cast<cc3d_dim_t>(threadNum);
            //extra workload - will be distributed evenly among first processes
            cc3d_dim_t extraWL =
                    yExtent % static_cast<cc3d_dim_t>(threadNum);
            ...
            // number of threads in this case is the same as the dimension in the y direction
            unsigned int threadNum = 1;
            if (fieldDim.y > 0) {
                threadNum = static_cast<unsigned int>(fieldDim.y);
            }


            cc3d_dim_t wL = 1; //base workload for single processor
    ...
    if (static_cast<unsigned int>(fieldDim.z) >= optimalNumberOfThreads) {
        //z dimension is large enough so that we can divide lattice using optimal division scheme
        ...
        const cc3d_dim_t zExtent = fieldDim.z;

        const cc3d_dim_t wL =
                zExtent / static_cast<cc3d_dim_t>(threadNum);
...
unsigned int ParallelUtilsOpenMP::getNumberOfWorkNodesPotts() {
    return static_cast<unsigned int>(pottsPartitionVec.size());
}

unsigned int ParallelUtilsOpenMP::getMaxNumberOfWorkNodesPotts() {
    return static_cast<unsigned int>(getMaxNumberOfWorkNodesFESolver());
}

unsigned int ParallelUtilsOpenMP::getMaxNumberOfWorkNodes() {
    return omp_get_max_threads_u();
}

unsigned int ParallelUtilsOpenMP::getNumberOfWorkNodesPottsWithBoxWatcher() {
    //THIS HAS TO BE CORRECTED
    return static_cast<unsigned int>(pottsPartitionVec.size());
}

unsigned int ParallelUtilsOpenMP::getNumberOfSubgridSectionsPotts() {
    return static_cast<unsigned int>(pottsPartitionVec[0].size());
}

std::pair<Dim3D, Dim3D> ParallelUtilsOpenMP::getPottsSection(unsigned int _workNodeNum,
                                                             unsigned int _subgridSectionNumber) {
...
void ParallelUtilsOpenMP::prepareParallelRegionPotts(bool _useBoxWatcher) {
    omp_set_dynamic(0);
    if (_useBoxWatcher) {
        omp_set_num_threads_u(getNumberOfWorkNodesPottsWithBoxWatcher());
    } else {
        omp_set_num_threads_u(getNumberOfWorkNodesPotts());
    }
}


std::vector<unsigned int> ParallelUtilsOpenMP::calculatePartitioning(unsigned int _numberOfProcessors,
                                                                     bool _quasi2DFlag) {
    ...
    unsigned int primeFactorsSize = static_cast<unsigned int>(primeFactors.size());
    const int pfSize = static_cast<int>(primeFactorsSize);
    ...
    if (_quasi2DFlag) {
        for (unsigned int i = 0; i < primeFactorsSize; ++i) {
            const auto idx =
                    static_cast<size_t>(pfSize - static_cast<int>(i) - 1);

            const size_t slot =
                    static_cast<size_t>(2) - static_cast<size_t>(i % 2);

            partitionVec[slot] *= primeFactors[idx];
        }
    } else {
        for (unsigned int i = 0; i < primeFactorsSize; ++i) {
            const auto idx =
            ...
            const size_t slot =
                    static_cast<size_t>(2) - static_cast<size_t>(i % 3);
...
void ParallelUtilsOpenMP::generateLatticePartition(unsigned int _numberOfProcessors, bool _quasi2DFlag,
                                                   std::vector<unsigned int> _dimIndexOrderedVec) {
    ...
    if (_numberOfProcessors <= numArrayElements - 1) {
        //requested less processors than max number of prepared partitions
    } else {
        //requested more processors than we expect - using algorothmic partitioning
    }
    ...
    if (_quasi2DFlag) {
        ...
        if (_numberOfProcessors <= numArrayElements - 1) {
            ...
            partitionVec[0] = static_cast<unsigned int>(latticeGridPartition2D[_numberOfProcessors][0]);
            partitionVec[1] = static_cast<unsigned int>(latticeGridPartition2D[_numberOfProcessors][1]);
            partitionVec[2] = static_cast<unsigned int>(latticeGridPartition2D[_numberOfProcessors][2]);
        } else {
            //requested more processors than we expect - using algorothmic partitioning
        ...
        if (_numberOfProcessors <= numArrayElements - 1) {
            ...
            partitionVec[0] = static_cast<unsigned int>(latticeGridPartition3D[_numberOfProcessors][0]);
            partitionVec[1] = static_cast<unsigned int>(latticeGridPartition3D[_numberOfProcessors][1]);
            partitionVec[2] = static_cast<unsigned int>(latticeGridPartition3D[_numberOfProcessors][2]);
        } else {
            //requested more processors than we expect - using algorothmic partitioning
            partitionVec = calculatePartitioning(_numberOfProcessors, _quasi2DFlag);
        }
...
void ParallelUtilsOpenMP::calculatePottsPartition() {
    ...
    //
    //
    // vector<cc3d_dim_t> dimVecTmp=dimVec; //have to make a copy because dimVecTmp will be altered
    //
    // //first we determine minimum dimension and its position in the vector
    // auto minDimCoord=*min_element(dimVecTmp.begin(),dimVecTmp.end());
    // //unsigned int indexMin=distance(dimVecTmp.begin(),min_element(dimVecTmp.begin(),dimVecTmp.end()));
    // auto indexMin =
    // 	static_cast<size_t>(
    // 		std::distance(dimVecTmp.begin(),
    // 			std::min_element(dimVecTmp.begin(), dimVecTmp.end())));
    ...
    //
    //
    // auto maxDimCoord=*max_element(dimVecTmp.begin(),dimVecTmp.end());
    // /*unsigned int indexMax=distance(dimVecTmp.begin(),max_element(dimVecTmp.begin(),dimVecTmp.end()));*/
    // auto indexMax = static_cast<size_t>(distance(dimVecTmp.begin(), max_element(dimVecTmp.begin(), dimVecTmp.end())));
    //
    //
    //
    // //finding index of "middle" dimension
    // size_t indexMiddle = 0;
    // cc3d_dim_t middleDimCoord=0;
    // for(int i = 0 ; i < 3; ++i){
    ...
    std::vector<cc3d_dim_t> dimVec;
    std::vector<unsigned int> dimIndexOrderedVec(3, 0);

    dimVec.push_back(fieldDim.x);
    dimVec.push_back(fieldDim.y);
    dimVec.push_back(fieldDim.z);

    std::vector<cc3d_dim_t> dimVecTmp = dimVec;

    const cc3d_dim_t minDimCoord =
            *std::min_element(dimVecTmp.begin(), dimVecTmp.end());

    const auto indexMin =
            static_cast<size_t>(std::distance(dimVecTmp.begin(),
                                              std::min_element(dimVecTmp.begin(), dimVecTmp.end())));

    dimVecTmp[indexMin] = 0;

    const cc3d_dim_t maxDimCoord =
            *std::max_element(dimVecTmp.begin(), dimVecTmp.end());
    ...
    dimIndexOrderedVec[0] = static_cast<unsigned int>(indexMin);
    dimIndexOrderedVec[1] = static_cast<unsigned int>(indexMiddle);
    dimIndexOrderedVec[2] = static_cast<unsigned int>(indexMax);
    ...
    CC3D_Log(LOG_TRACE) << "minDimCoord=" << minDimCoord << " indexMin=" << indexMin << " maxDimCoord=" << maxDimCoord
 << " indexMax=" << indexMax << " middleDimCoord=" << middleDimCoord << " indexMiddle=" << indexMiddle;

    generateLatticePartition(optimalNumberOfThreads, quasi2D, dimIndexOrderedVec);
}


void ParallelUtilsOpenMP::partitionLattice(unsigned int minDimGridPoints, unsigned int middleDimGridPoints,
                                           unsigned int maxDimGridPoints,
                                           std::vector<unsigned int> _dimIndexOrderedVec) {
    ...
    auto minDimCoord = fieldDim[indexMin];
    auto middleDimCoord = fieldDim[indexMiddle];
    auto maxDimCoord = fieldDim[indexMax];
    ...
    vector<cc3d_dim_t> minDimDivisionVec;
    vector<cc3d_dim_t> middleDimDivisionVec;
    vector<cc3d_dim_t> maxDimDivisionVec;
    ...
    for (size_t i = 0; i < minDimGridPoints; ++i) {
        // minDimDivisionVec.push_back(i*(minDimCoord/minDimGridPoints));
        const cc3d_dim_t stepMin =
                minDimCoord / static_cast<cc3d_dim_t>(minDimGridPoints);

        minDimDivisionVec.push_back(
            static_cast<cc3d_dim_t>(i) * stepMin);
    }

    for (size_t i = 0; i < middleDimGridPoints; ++i) {
        const cc3d_dim_t stepMid =
                middleDimCoord / static_cast<cc3d_dim_t>(middleDimGridPoints);

        middleDimDivisionVec.push_back(static_cast<cc3d_dim_t>(i) * stepMid);
    }

    for (size_t i = 0; i < maxDimGridPoints; ++i) {
        const cc3d_dim_t stepMax =
                maxDimCoord / static_cast<cc3d_dim_t>(maxDimGridPoints);

        maxDimDivisionVec.push_back(static_cast<cc3d_dim_t>(i) * stepMax);
    }

    if (minDimGridPoints == 1) {
        //each grid will be divided into 4 subgrids in quasi-2D lattice division
        pottsPartitionVec.assign(middleDimDivisionVec.size() * maxDimDivisionVec.size(),
                                 vector<pair<Dim3D, Dim3D> >(8, make_pair(Dim3D(), Dim3D())));
    } else {
        //each grid will be divided into 8 subgrids in quasi-3D lattice division
        pottsPartitionVec.assign(minDimDivisionVec.size() * middleDimDivisionVec.size() * maxDimDivisionVec.size(),
                                 vector<pair<Dim3D, Dim3D> >(8, make_pair(Dim3D(), Dim3D())));
    }

    size_t gridId = 0;
    for (size_t i = 0; i < minDimDivisionVec.size(); ++i)
        for (size_t j = 0; j < middleDimDivisionVec.size(); ++j)
            for (size_t k = 0; k < maxDimDivisionVec.size(); ++k) {
                Dim3D minDim;
                Dim3D maxDim;
                ...
                maxDim[indexMiddle] = (j < middleDimDivisionVec.size() - 1
                                           ? middleDimDivisionVec[j + 1]
                                           : fieldDim[indexMiddle]);
    ...
    size_t gridIdMax = gridId;

    for (size_t gid = 0; gid < gridIdMax; ++gid) {
        //Grid boundaries
        ...
        for (unsigned int i = 0; i < minIndexSubgrids; ++i) //minIndex
            for (unsigned int j = 0; j < 2; ++j) //middleIndex
                for (unsigned int k = 0; k < 2; ++k) {
                    const auto i_cc = static_cast<cc3d_dim_t>(i);
                    const auto j_cc = static_cast<cc3d_dim_t>(j);
                    const auto k_cc = static_cast<cc3d_dim_t>(k);
                    const cc3d_dim_t halfMin =
                        (maxDim[indexMin] - minDim[indexMin]) / 2;
                    const cc3d_dim_t halfMid =
                        (maxDim[indexMiddle] - minDim[indexMiddle]) / 2;

                    const cc3d_dim_t halfMax =
                        (maxDim[indexMax] - minDim[indexMax]) / 2;
                    ...
                    if (minIndexSubgrids == 1) {
                        ...
                        minSubDim[indexMin] = minDim[indexMin] + i_cc * halfMin;

                        if (i < 1) {
                            maxSubDim[indexMin] = minDim[indexMin] + (i_cc + 1) * halfMin;
                        } else {
                            maxSubDim[indexMin] = maxDim[indexMin];
                        }
                    }
                    ...
                    // ----- MIDDLE dimension -----
                    minSubDim[indexMiddle] =
                        minDim[indexMiddle] + j_cc * halfMid;

                    if (j < 1) {
                        maxSubDim[indexMiddle] =
                            minDim[indexMiddle] + (j_cc + 1) * halfMid;
                    } else {
                    ...
                    // ----- MAX dimension -----
                    minSubDim[indexMax] =
                        minDim[indexMax] + k_cc * halfMax;

                    if (k < 1) {
                        maxSubDim[indexMax] =
                            minDim[indexMax] + (k_cc + 1) * halfMax;
                    } else {
                        maxSubDim[indexMax] = maxDim[indexMax];
                    }

                    pottsPartitionVec[gid][subgridId] =
                        make_pair(minSubDim, maxSubDim);
 ...
 //                        if (i < 1) {
 //                            maxSubDim[indexMin] =
 //                                    minDim[indexMin] + (i + 1) * ((maxDim[indexMin] - minDim[indexMin]) / 2);
 //                        } else {
 //                            maxSubDim[indexMin] = maxDim[indexMin];
 //                        }
 //                    }
 ...
 //                    minSubDim[indexMiddle] = minDim[indexMiddle] + j * (
 //                                                 (maxDim[indexMiddle] - minDim[indexMiddle]) / 2);
 //                    if (j < 1) {
 //                        maxSubDim[indexMiddle] =
 //                                minDim[indexMiddle] + (j + 1) * ((maxDim[indexMiddle] - minDim[indexMiddle]) / 2);
 //                    } else {
 //                        maxSubDim[indexMiddle] = maxDim[indexMiddle];
 //                    }
 //
 ...
 //                    CC3D_Log(LOG_DEBUG) << " GID=" << gid << " subgridId=" << subgridId << " minSubDim=" << minSubDim <<
 // " maxSubDim=" << maxSubDim;
 //                    ++subgridId;
                }
    }
