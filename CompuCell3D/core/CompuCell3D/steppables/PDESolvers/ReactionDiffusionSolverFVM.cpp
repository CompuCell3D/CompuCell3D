...
// macro to ensure log is enabled only when debugging
#ifdef DEBUG
...
void ReactionDiffusionSolverFVM::init(Simulator *_simulator, CC3DXMLElement *_xmlData) {
	...
	log("*******************************")
	log("* Begin RDFVM initialization! *")
	log("*******************************")
	// cerr << "*******************************" << endl;
	...
	log( "  Boundary pixel tracker plugin...")
	...
	log("Getting static RDFVM Solver inputs...")
	//cerr << "Getting static RDFVM Solver inputs..." << endl;

	//		Cell types

	log("Getting cell types...")
	...
	if (xmlData->findElement("DeltaT")) {
		...
		log("   Got time discretization: " << incTime << " " << getUnitsTime() << "/step")
	...
	if (xmlData->findElement("DeltaX")) {
		...
		log("   Got x-dimension discretization: " << DeltaX << " m")
		...
		log("   Got y-dimension discretization: " << DeltaY << " m")
		...
		log("   Got z-dimension discretization: " << DeltaZ << " m")
	...
	for (unsigned int fieldIndex = 0; fieldIndex < numFields; ++fieldIndex) {
		...
		log("   Registering field with Simulator...")
		...
		if (dDataEl) {
			...
			log("   Got diffusion constant: " << constantDiffusionCoefficientsVec[fieldIndex] << " m2/s")
		...
		if (dData->findElement("DiffusivityFieldInMedium")) {
			if (diffusionDefined) { 
				log("Warning: duplicate diffusion mode. Overwriting with DiffusivityFieldInMedium" )
				//cerr << "Warning: duplicate diffusion mode. Overwriting with DiffusivityFieldInMedium" << endl; 
				}
			else { 
				log("   Got diffusivity field in medium. Diffusivity field is named: " + fieldName + diffusivityFieldSuffixStd)
		...
		if (dData->findElement("DiffusivityFieldEverywhere")) {
			if (diffusionDefined) { 
				log("Warning: duplicate diffusion mode. Overwriting with DiffusivityFieldEverywhere")
				//cerr << "Warning: duplicate diffusion mode. Overwriting with DiffusivityFieldEverywhere" << endl;
				 }
			else { 
				log("   Got diffusivity field everywhere. Diffusivity field is named: " + fieldName + diffusivityFieldSuffixStd)
		...
		if (dData->findElement("InitialConcentrationExpression")) {
			...
			log("   Got initial concentration expression: " + initialExpressionStrings[fieldIndex])
		...
		for (CC3DXMLElement *typeData : dData->getElements("DiffusionCoefficient")) {
			...
			if (cellTypeNameToIndexMap_itr != cellTypeNameToIndexMap.end()) {
				...
				log("   Got cell type (" << cellTypeName << ") diffusivity: " << typeDiffC << " m2/s")
		...
		for (CC3DXMLElement *typeData : dData->getElements("PermIntCoefficient")) {
			...
			if (cellType1NameToIndexMap_itr != cellTypeNameToIndexMap.end() && cellType2NameToIndexMap_itr != cellTypeNameToIndexMap.end()) {
				...
				log("   Got cell type (" << cellType1Name << ", " << cellType2Name << ") interface permeation coefficient: " << permC << " m/s")
		...
		for (CC3DXMLElement *typeData : dData->getElements("PermIntBias")) {
			...
			if (cellType1NameToIndexMap_itr != cellTypeNameToIndexMap.end() && cellType2NameToIndexMap_itr != cellTypeNameToIndexMap.end()) {
				...
				log( "   Got cell type (" << cellType1Name << ", " << cellType2Name << ") interface bias coefficient: " << biasC)
		...
		if (rData) {
			...
			if (rDataEl) {
				...
				log( "   Got reaction expression symbol: " << fieldSymbolsVec[fieldIndex])
			...
			if (rData->findElement("ExpressionMult")){
				...
				for (CC3DXMLElement *expData : rData->getElements("ExpressionMult")) {
					...
					log("   Got multiplier reaction expression: " << expStr)
			...
			if (rData->findElement("ExpressionIndep")) {
				...
				for (CC3DXMLElement *expData : rData->getElements("ExpressionIndep")) {
					...
					log("   Got independent reaction expression: " << expStr )
		...
		log("   Collecting boundary conditions...")
	...
	for (unsigned int fieldIndex = 0; fieldIndex < numFields; ++fieldIndex) {
		...
		if (useConstantDiffusivityBool[fieldIndex]) {
			...
			log("constant diffusivity by type.")
			...
			log("diffusivity field in medium.")
			...
			log("diffusivity field everywhere.")
	...
	for (unsigned int nIdx = 0; nIdx <= maxNeighborIndex; ++nIdx) {
		...
		log( "   Processing surface for neighbor relative offset (" << offset.x << ", " << offset.y << ", " << offset.z << ") -> ")
		...
		if (offset.x > 0) {
			...
			log("Warning: assuming a neighbor surface map: " << nIdx)
	...
	log( "Loading boundary conditions...")
	...
	for (unsigned int fieldIndex = 0; fieldIndex < concentrationFieldNameVector.size(); ++fieldIndex) {
		...
		log("Loading boundary conditions for field " + fieldName)
		...
		if (bc_itr != bcElementCollector.end()) {
			...
			for (CC3DXMLElement *bcEl : bcPlaneElementList) {
				...
				for (CC3DXMLElement *bcElSpec : bcEl->children) {
					...
					log("   Got boundary condition " << bcTypeName << " for " << surfaceName << " with value " << bcVal)
					log("      Loading to surface index " << surfaceIndex)
					log( " for dimension index " << dimIndex)
	...
	log("Initializing reaction expressions...")
	...
	for (unsigned int fieldIndex = 0; fieldIndex < numFields; ++fieldIndex) {
		log("Constructing reaction expressions for " + concentrationFieldNameVector[fieldIndex])
		...
		if (fieldExpressionStringsDiag[fieldIndex].size() == 0) { expDiag = "0.0"; }
			...
			log("   Multiplier function: " + expDiag)
		...
		if (fieldExpressionStringsOffDiag[fieldIndex].size() == 0) { expOffDiag = "0.0"; }
			...
			log("   Independent function: " + expOffDiag)
	...
	log("Initializing concentrations...")
	...
	if (autoTimeSubStep) {
		log("RDVFM got automatic time sub-stepping.")
	...
	if (simpleMassConservation) { 
		log("RDVFM got simple mass conservation.")
...
void ReactionDiffusionSolverFVM::handleEvent(CC3DEvent & _event) {
	...
	//if (_event.id == LATTICE_RESIZE) {
		...
		if (simpleMassConservation) {
			log("RDFVM Initializing pixel tracker data...")
...
void ReactionDiffusionSolverFVM::step(const unsigned int _currentStep) {
	...
	if (simpleMassConservation) {
		...
		log( "   Simple mass conservation: updating fields...")
	...
	while (intTime < incTime) {
		...
		if (autoTimeSubStep) {
			...
			log( "      Integrating with maximum stable time step... ")
			...
			log("calculating maximum stable time step... ")
			...
			log("      Integrating with fixed time step... ")
		...
		log("done: " << physTime / unitTimeConv << " " << getUnitsTime())
	...
	if (simpleMassConservation) {
		...
		log("   Simple mass conservation: updating properties..." )
...
void ReactionDiffusionSolverFVM::update(CC3DXMLElement *_xmlData, bool _fullInitFlag) {
	...
	if (!simpleMassConservation_old && simpleMassConservation) {
		...
		log("RDFVM enabling simple mass conservation...")
		...
		log("RDFVM disabling simple mass conservation...")
...
void ReactionDiffusionSolverFVM::initializeFVs(Dim3D _fieldDim) {
	...
	log("Constructing lattice with " << _fieldDim.x*_fieldDim.y*_fieldDim.z << " sites...")
	...
	log("Loading field expressions...")
	//cerr << "Loading field expressions..." << endl;

	loadFieldExpressions();

	log("Setting initial FV diffusivity method...")
	...
	log("Applying basic boundary conditions...")
...
double ReactionDiffusionSolverFVM::getCellDiffusivityCoefficient(const CellG * _cell, unsigned int _fieldIndex) {
	...
	if (diffusivityCoefficients.size() < _fieldIndex) { 
		...
		log("Initializing cell diffusivity coefficient on the fly...")
...
std::vector<double> ReactionDiffusionSolverFV::diffusiveSurfaceFlux(unsigned int _fieldIndex, unsigned int _surfaceIndex, ReactionDiffusionSolverFV *_nFv) {
	if (_nFv == nullptr) {
		log("Warning: diffusive surface flux for an unconnected FV pair!" )
...
std::vector<double> ReactionDiffusionSolverFV::permeableSurfaceFlux(unsigned int _fieldIndex, unsigned int _surfaceIndex, ReactionDiffusionSolverFV *_nFv) {
	if (_nFv == nullptr) {
		log("Warning: permeable surface flux for an unconnected FV pair!")
