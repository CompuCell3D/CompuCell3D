...
FluctuationCompensator::~FluctuationCompensator() {
	for (cellCompensatorDataItr = cellCompensatorData.begin(); cellCompensatorDataItr != cellCompensatorData.end(); ++cellCompensatorDataItr) {
...
void FluctuationCompensator::applyCorrections() {
	...
	std::vector<std::tuple<FluctuationCompensatorCellData *, std::set<PixelTrackerData> > > cellDataVec(numCells);
	unsigned int cellIdx = 0;
	for (CellInventory::cellInventoryIterator itr = cellInventory->cellInventoryBegin(); itr != cellInventory->cellInventoryEnd(); ++itr) {
		CellG *cell = cellInventory->getCell(itr);
		FluctuationCompensatorCellData *fccd = cellCompensatorData[cell];
		std::set<PixelTrackerData> pixelSet = pixelTrackerPlugin->getPixelTrackerAccessorPtr()->get(cell->extraAttribPtr)->pixelSet;
...
#pragma omp parallel
	{
		...
		for (int workerNum = 0; workerNum < numWorkers; ++workerNum) {
			...
			for (auto ptd : pixelSetLocal)
				for (int fieldIdx = 0; fieldIdx < diffusibleFields.size(); ++fieldIdx)
					diffusibleFields[fieldIdx]->set(ptd.pixel, (correctionFactorsLocal[fieldIdx] * diffusibleFields[fieldIdx]->get(ptd.pixel)));
		}
		...
		{
			std::tuple<FluctuationCompensatorCellData *, std::set<PixelTrackerData> > cellData = cellDataVec[cellIdx];
...
std::vector<float> FluctuationCompensator::totalCellConcentration(const CellG *_cell) { return totalPixelSetConcentration(getCellPixelVec(_cell)); }

std::vector<float> FluctuationCompensator::totalMediumConcentration() { return totalPixelSetConcentration(getMediumPixelVec()); }
...
std::vector<float> FluctuationCompensator::totalPixelSetConcentration(std::vector<Point3D> _pixelVec) {
	...
	for (unsigned int pixelIndex = 0; pixelIndex < _pixelVec.size(); ++pixelIndex) {
		for (unsigned int vecIndex = 0; vecIndex < numFields; ++vecIndex) { sumVec[vecIndex] += diffusibleFields[vecIndex]->get(_pixelVec[pixelIndex]); }
	}
	return sumVec;
}

FluctuationCompensatorCellData *FluctuationCompensator::getFluctuationCompensatorCellData(CellG *_cell, bool _fullInit) {
