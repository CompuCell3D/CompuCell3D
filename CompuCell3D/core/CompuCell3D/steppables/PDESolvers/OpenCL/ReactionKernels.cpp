...
std::string
genReactionKernelFunc(std::ofstream &f, size_t ind, std::vector <fieldNameAddTerm_t> const &fieldNameAddTerms) {
...
string genReactionKernels(std::vector < fieldNameAddTerm_t >
...
funcNames.
push_back(genReactionKernelFunc(f, i, fieldNameAddTerms)
);
}
...
f<<"\tint4 ind3d={get_global_id(0), get_global_id(1), get_global_id(2), 0};"<<
endl;
...
++i){
f<<"\tfloat field"<<i<<"=(g_newField+"<<"stride*"<<i<<")[ind];"<<
endl;
}
...
"(g_prevField+stride*"<<i<<")[ind]-(g_result+stride*"<<i<<")[ind]+"<<"dt*"<<funcNames[i]<<"("<<arg.
