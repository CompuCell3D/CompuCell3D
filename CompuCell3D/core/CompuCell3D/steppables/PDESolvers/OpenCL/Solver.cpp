...
//	std::cerr<<"Solver::ctor\n";
	...
	for(unsigned char i=0; i<getFieldsCount(); ++i){
		...
		cl_int retCode;
		cl_mem subBuffer=clCreateSubBuffer(outBuffer, CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &subBufferInfo, &retCode);
		ASSERT_OR_THROW("Can't make an output subbuffer", retCode==CL_SUCCESS);
		m_outFieldSubBuffers.push_back(subBuffer);
		
		m_ims.push_back(new ImplicitMatrix(oclHelper, solverParams[i], d_cellTypes, subBuffer, boundaryConditions, pathToKernels));
...
void Solver::applyBCToRHS(viennacl::vector<float> const &rhs)const{
	...
	for(unsigned char i=0; i<getFieldsCount(); ++i){
		...
		cl_int retCode;
		cl_mem subBuffer=clCreateSubBuffer(rhs.handle().opencl_handle().get(), CL_MEM_READ_WRITE, CL_BUFFER_CREATE_TYPE_REGION, &subBufferInfo, &retCode);
