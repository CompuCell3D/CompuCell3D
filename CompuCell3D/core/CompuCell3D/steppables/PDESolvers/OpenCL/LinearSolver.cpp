...
LinearSolver::LinearSolver(OpenCLHelper const &oclHelper,
    ...
    unsigned char fieldsCount, std::string const &pathToKernels):Solver(oclHelper, solverParams, d_cellTypes, boundaryConditions, fieldsCount, pathToKernels)
{
...
LinearSolver::NewField(float dt, viennacl::vector<float> const &v_oldField, NLSParams const &nlsParams){
	...
	viennacl::linalg::cg_tag solver_tag=viennacl::linalg::cg_tag(nlsParams.linear_.tol_, nlsParams.linear_.maxIterations_);
