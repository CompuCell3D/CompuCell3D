...
template <class T> 
inline 
std::pair<size_t, T>  max(viennacl::vector<T> const &v){
	std::vector<T> h_v(v.size());
	viennacl::copy(v, h_v);
	std::vector<T>::const_iterator it=std::max_element(h_v.begin(), h_v.end());
	return std::make_pair(std::distance(h_v.cbegin(), it), *it);
}

template <class T> 
inline 
std::pair<size_t, T>  min(viennacl::vector<T> const &v){
	std::vector<T> h_v(v.size());
	viennacl::copy(v, h_v);
...
template <class T> 
inline 
std::pair<bool, size_t>  isFinite(viennacl::vector<T> const &v){
	std::vector<T> h_v(v.size());
	viennacl::copy(v, h_v);
	auto notFiniteWrap=std::not1(std::ptr_fun(_finite));
	auto itf=std::find_if(h_v.begin(), h_v.end(), notFiniteWrap);
	return std::make_pair(itf==h_v.end(), std::distance(h_v.begin(), itf));
}
