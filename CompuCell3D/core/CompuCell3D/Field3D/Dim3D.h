...
namespace CompuCell3D {
    ...
    public:
        ...
        Dim3D(const cc3d_dim_t x, const cc3d_dim_t y, const cc3d_dim_t z) : Point3D(x, y, z) {}
        ...
        /// Add the coordinates of pt to this Dim3D.
        Dim3D &operator+=(const Dim3D pt);

        /// Subtract the coordinates of pt to this Dim3D.
        Dim3D &operator-=(const Dim3D pt);
    ...
    inline cc3d_dim_t &Dim3D::operator[](int _idx) {
        if (!_idx) {
            ...
            return y;
        } else { // no error checking; out of range returns z
            return z;
        }
    }
    ...
    /// Overloads the operator std::string + Dim3D.
    inline std::string operator+(const std::string& s, const Dim3D& pt) {
        return s + "(" + std::to_string(pt.x) + "," +
               std::to_string(pt.y) + "," +
               std::to_string(pt.z) + ")";
    }
    // inline std::string operator+(const std::string s, const Dim3D pt) {
    //     return s + "(" + std::to_string(static_cast<long long>(pt.x)) + "," +
    //            std::to_string(static_cast<long long>(pt.y)) + "," +
    //            std::to_string(static_cast<long long>(pt.z)) + ")";
    // }
