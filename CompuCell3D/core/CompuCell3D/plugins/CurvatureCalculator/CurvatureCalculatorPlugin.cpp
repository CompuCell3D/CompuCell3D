...
void CurvatureCalculatorPlugin::extraInit(Simulator *simulator) {
	...
	Plugin *nt_plugin = Simulator::pluginManager.get("NeighborTracker", &neighborTrackerAlreadyRegisteredFlag);  //this will load NeighborTracker plugin if it is not already loaded
	...
	boundary_pixel_tracker_plugin = (BoundaryPixelTrackerPlugin*)Simulator::pluginManager.get("BoundaryPixelTracker", &boundaryPixelTrackerAlreadyRegisteredFlag);
