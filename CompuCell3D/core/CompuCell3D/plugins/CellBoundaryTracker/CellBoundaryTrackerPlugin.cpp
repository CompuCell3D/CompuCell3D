            ...
            for (int x = 0; x < fieldDim.x; ++x) {
                ...
                if (set_of_visited_cells.find(currentCellPtr) != set_of_visited_cells.end()) {
                    ...
                    if (set_NSD_local.size() != set_NSD_ref.size()) {
                        cerr << "Sets have different sizes - orig:" << set_NSD_ref.size() << " local:"
                             << set_NSD_local.size() <<
                             " . Exining..." << endl;
                        exit(0);
                    ...
                    for (set<NeighborSurfaceData>::iterator sitr = set_NSD_ref.begin();
                        ...
                        if (sitr_local->neighborAddress != sitr->neighborAddress) {
                            cerr << "Neighbor addresses - orig: " << sitr->neighborAddress << " local:"
                                 << sitr_local->neighborAddress <<
                                 " do not match. Exiting " << endl;
                            exit(0);
                        }
                        if (sitr_local->commonSurfaceArea != sitr->commonSurfaceArea) {
                            cerr << "Neighbor commonSurfaceArea - orig: " << sitr->commonSurfaceArea << " local:"
                                 << sitr_local->commonSurfaceArea <<
                                 " do not match. Exiting " << endl;
                            exit(0);
                        }
                ...
                if (isInBoundary) {
                    ...
                    if (bdsitr == set_ref.end()) {
                        cerr << "Requested pixel:" << currentPtIndex
                             << " was not found in the boundary set. Boundary corrupted. Exiting..." << endl;
                        ...
                        cerr << "Number of fN's is " << bdsitr->numberOfForeignNeighbors << " but allowed value is 26 "
                             << endl;
                        exit(0);
