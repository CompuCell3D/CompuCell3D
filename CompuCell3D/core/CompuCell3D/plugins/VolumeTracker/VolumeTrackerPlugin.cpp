...
void VolumeTrackerPlugin::field3DChange(const Point3D &pt, CellG *newCell, CellG *oldCell) {
    ...
    if (newCell) {
        ...
        // computing pressure - handling cases when we use global, by cell type or local volume energy parameters
        if (potts->areVolumeParametersLocal()) {
            newCell->pressure = -newCell->lambdaVolume * (newCell->volume - newCell->targetVolume);
        } else {
            newCell->pressure =
                    -potts->getLambdaVolume(newCell->type) * (newCell->volume - potts->getTargetVolume(newCell->type));
        }
    }
    ...
    if (oldCell) {
        ...
        // computing pressure - handling cases when we use global, by cell type or local volume energy parameters
        if (potts->areVolumeParametersLocal()) {
            oldCell->pressure = -oldCell->lambdaVolume * (oldCell->volume - oldCell->targetVolume);
        } else {
            oldCell->pressure =
                    -potts->getLambdaVolume(oldCell->type) * (oldCell->volume - potts->getTargetVolume(oldCell->type));
        }
    }
