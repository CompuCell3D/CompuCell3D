...
class PluginWidget:
    ...
    # SurfacePlugin widgets - now matches VolumePlugin
...
class PottsWidget:
    ...
    def create_ui(self):
        ...
        lattice_type_box = widgets.Box([self.widgets["lattice_type"]])
        advanced_row1 = HBox([neighbor_order_box, lattice_type_box])
        advanced_row1.add_class('vbox-row-spacing')

        offset_box = widgets.Box([self.widgets["offset"]])
        seed_box = widgets.Box([self.widgets["random_seed"]])
        energy_box = widgets.Box([self.widgets["energy_calculator"]])
        advanced_row2 = HBox([offset_box, seed_box, energy_box])
...
class SpecificationSetupUI:
    def __init__(self):
        ...
        self.create_ui()
        self.setup_event_handlers()
        self.check_ready_state()  # Initial ready state check
    ...
    def check_ready_state(self):
        # Enable run button if required specs are set
        required_ready = (
    ...
    def create_ui(self):
        ...
        run_button_box = VBox([self.widgets["run_button"]])
        run_button_box.add_class('vbox-no-margin')
        button_row = HBox([reset_button_box, run_button_box])
    ...
    def run_simulation(self):
        ...
        # Here you would call the actual simulation execution code