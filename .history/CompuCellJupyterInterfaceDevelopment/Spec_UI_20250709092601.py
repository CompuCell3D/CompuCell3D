...
def get_defaults():
    return {
        ...
        "CellType": [{"Cell type": "Medium", "freeze": False}], # I think this error is because actually the default for celltype is Medium, 0, False and that applies to cell type, cell type id, and freeze. 
        "Plugins": {
...
class PluginWidget:
    ...
    def create_volume_widgets(self, saved_values):
        ...
        self.widgets["add_btn"].on_click(self.add_volume_row)
        
        # Build UI
        self.update_volume_ui()
    
    def add_volume_row(self, _):
        """Add a new row to VolumePlugin configuration"""
    ...
    def create_surface_widgets(self, saved_values):
        ...
        self.widgets["add_btn"].on_click(self.add_surface_row)
        
        # Build UI
        self.update_surface_ui()
    
    def add_surface_row(self, _):
        """Add a new row to SurfacePlugin configuration"""
        # Get default values from class
        default_params = SurfacePlugin().spec_dict.get("params", [])
        default_values = default_params[0] if default_params else {
            "CellType": "Medium",
...
class CellTypeWidget:
    ...
    def reset(self, _=None):
        ...
        for entry in DEFAULTS["CellType"]:
            self.add_entry(entry["Cell type"], entry.get("free"))
        self.update_celltype_display()
