    ...
    for methodName in ["handleStdMsg", "handleErrMsg", "handleDbgMsg"]:
        ...
        try:
            ...
            raise MessagingException(
                "The class " + handler.__class__.__name__ + " is missing a " + methodName + " method")
...
def stdMsg(*args):
    stdStr = string.join(list(map(str, args)), " ")
...
def errMsg(*args):
    errStr = "Error in " + getCallString(1) + " : " + ''.join(list(map(str, args)) +[" "])
...
def dbgMsg(*args):
    ...
    errStr = getCallString(1) + " : " + ''.join(list(map(str, args)) + [" "])
