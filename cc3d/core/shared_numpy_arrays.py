...
def get_shared_numpy_array_as_cc3d_field(shape:Tuple, dtype=np.float32):
    if dtype in (np.float64, ):
...
def register_shared_numpy_array(field_name: str, simulator, dtype=np.float32):
    ...
    array, field = get_shared_numpy_array_as_cc3d_field(shape=(dim.x, dim.y, dim.z), dtype=dtype)
