# the call has to be showTweditWindowInForeground has to be done from the twedit (foregraound process) and not from EditorWindow (not a foreground process) 
# because otherwise windows will not allow this call to succees and will return error code 


# PERHAPS A BETER SOLUTION IS TO STORE HANDLE TO TWEDIT ONCE IT IS STARTED INSTEAD OF SEARCHING EACH TIME WE OPEN NEW DOCUMENT

def win_enum_callback_twedit(hwnd, results):
    import win32gui
    import re
    exprChecker=re.compile(".*Twedit\+\+$")
    if exprChecker.match(win32gui.GetWindowText(hwnd)):
