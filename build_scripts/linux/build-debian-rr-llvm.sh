# example command ./build-debian-rr-llvm.sh -s=~/RR_LLVM -p=~/install_projects/RR_LLVM -c=4
...
export BUILD_ROOT=~/install_projects/RR_LLVM_build
export SOURCE_ROOT=~/RR_LLVM
export DEPENDENCIES_ROOT=~/install_projects
export INSTALL_PREFIX=~/install_projects/RR_LLVM
...
then
  ############# BUILDING CC3D
  mkdir -p $BUILD_ROOT/RR_LLVM
  cd $BUILD_ROOT/RR_LLVM


  run_and_watch_status RR_CMAKE_CONFIG cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX:PATH=${INSTALL_PREFIX}  -DTHIRD_PARTY_INSTALL_FOLDER:PATH=${INSTALL_PREFIX}  -DCMAKE_BUILD_TYPE:STRING=Release $SOURCE_ROOT 
