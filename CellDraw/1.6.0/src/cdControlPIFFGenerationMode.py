...
class CDControlPIFFGenerationMode(QtGui.QWidget):
    ...
    def __init__(self,parent=None):
        ...
        #
        CDConstants.printOut( "___ - DEBUG ----- CDControlPIFFGenerationMode: __init__() 1", CDConstants.DebugTODO )

        self.layerSelectionGroupBox = QtGui.QGroupBox("PIFF Mode:")
        ...
        # placeholder for fixed raster PIFF Generation raster-size:
        self.placeHolderForFixedRasterSize = QtGui.QWidget()
        self.placeHolderForFixedRasterSize.setLayout(QtGui.QHBoxLayout())
        self.placeHolderForFixedRasterSize.layout().setContentsMargins(0,0,0,0)
        self.placeHolderForFixedRasterSize.layout().setSpacing(0)
        self.placeHolderForFixedRasterSize.layout().setAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter)
        ...
        self.layerSelectionGroupBox.layout().addWidget(self.placeHolderForFixedRasterSize)
        ...
        # call handleButtonGroupForPIFFGenerationClicked() every time a button is clicked in the "theButtonGroupForPIFFGenerationSelection"
        self.theButtonGroupForPIFFGenerationSelection.buttonClicked[int].connect( \
            self.handleButtonGroupForPIFFGenerationClicked)


        # ----------------------------------------------------------------
        #
        # QWidget setup (4) - create a QSpinBox for the fixed raster mode:

        # fixed raster PIFF Generation button:
        self.rasterWidthSpinBox = QtGui.QSpinBox()
        self.rasterWidthSpinBox.setMinimum(4)
        self.rasterWidthSpinBox.setValue(self.thePIFFFixedRasterWidth)
        lStringForToolTip = "PIFF Generation - Fixed Raster Size\n sets all PIFF cell regions\n to be made of same-sized square cells."
        self.rasterWidthSpinBox.setToolTip(lStringForToolTip)
        self.rasterWidthSpinBox.setStatusTip("PIFF Generation: save all cells in all regions as squares on a fixed raster")
        # call setFixedRasterWidth() every time self.rasterWidthSpinBox changes its value:
        self.rasterWidthSpinBox.valueChanged[int].connect(self.setFixedRasterWidth)

        self.placeHolderForFixedRasterSize.layout().addWidget(self.rasterWidthSpinBox)
