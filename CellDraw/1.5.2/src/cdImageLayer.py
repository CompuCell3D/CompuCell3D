...
class CDImageNP(QtCore.QObject):
    ...
    def __init__(self, pParent=None):
        ...
        # a string containing the full path to the directory holding the image:
        self.imageNPPathString = ""
        ...
        # the class global keeping track of the bit-flag modes for generating PIFF from displayed imported image:
        ...
        self.theProcessingModeForImageNP = (1 << CDConstants.ImageNPUseAreaCells)
        ...
        #     it'll be resized when the image is loaded, i.e.
        #     the actually used array will be set in resetNPDimensions()
        ...
        self.volumeNPArray = numpy.zeros( (1, 1, 1, 1), dtype=numpy.uint8 )
        self.edgeNPArray = numpy.zeros( (1, 1, 1, 1), dtype=numpy.uint8 )
        self.contoursNPArray = numpy.zeros( (1, 1, 1, 1), dtype=numpy.uint8 )
        ...
        self.edgeInNPIsReadyFlags = numpy.zeros( (1), dtype=numpy.bool )
        ...
        # the "theCurrentVolumeSliceImage" QImage is the selected volume slice image discretized from the file image:
        lBoringPixMap.fill( QtGui.QColor(QtCore.Qt.transparent) )
        self.theCurrentVolumeSliceImage = QtGui.QImage(lBoringPixMap)

        # the "theCurrentEdgeImage" QImage is the selected edge image detected from the file image:
        lBoringPixMap.fill( QtGui.QColor(QtCore.Qt.transparent) )
        self.theCurrentEdgeImage = QtGui.QImage(lBoringPixMap)

        # the "theCurrentContourImage" QImage is the selected 3D contour slice as detected from the file image:
        lBoringPixMap.fill( QtGui.QColor(QtCore.Qt.transparent) )
        self.theCurrentContourImage = QtGui.QImage(lBoringPixMap)
        ...
        CDConstants.printOut( "___ - DEBUG ----- CDImageNP.__init__() - edgeInNPIsReadyFlags == "+str(self.edgeInNPIsReadyFlags)+" " , CDConstants.DebugExcessive )
        CDConstants.printOut( "___ - DEBUG ----- CDImageNP.__init__() - contoursAreReadyFlag == "+str(self.contoursAreReadyFlag)+" " , CDConstants.DebugExcessive )
    ...
    def setImage(self, pImage=None):
        ...
        #    and all the entries in the contours/edge/imageInNPIsReadyFlags arrays:
        self.resetNPDimensions(lXdim, lYdim, lZdim, lImageChannels)

        # tell the CDImageNP object to use Region 2D Cell Contours from the image loaded from file:
        self.resetToOneProcessingModeForImageNP( CDConstants.ImageNPUseAreaCells )
...
#         self.normalizeAllImages()
    ...
    # ------------------------------------------------------------------
    # 2011 - Mitja: imageCurrentImageNP() creates theCurrentImage, theCurrentEdgeImage and theCurrentVolumeSliceImage
    ...
    def imageCurrentImageNP(self):
        ...
        lTmpOneLayerArray = self.volumeNPArray[self.theCurrentIndex]
        ...
        if (self.edgeInNPIsReadyFlags[self.theCurrentIndex] == False):
        ...
        lTmpOneLayerArray = self.edgeNPArray[self.theCurrentIndex]
        self.theCurrentEdgeImage = self.rgb2qimageWtoRandA(lTmpOneLayerArray)

        # if 3D contours have to be painted, check if the 3D contours have already been computed, and if not, do so now:
        if ( self.getAProcessingModeStatusForImageNP(CDConstants.ImageNPUse3DContours) ) and \
            (self.contoursAreReadyFlag == False):
        ...
        lTmpOneLayerArray = self.contoursNPArray[self.theCurrentIndex]
        self.theCurrentContourImage = self.rgb2qimageWtoGandA(lTmpOneLayerArray)
...
# 
#                 r = int ( self.imageNPArray[k, j, i, 0] )
#                 g = int ( self.imageNPArray[k, j, i, 1] )
#                 b = int ( self.imageNPArray[k, j, i, 2] )
#                 lColor.setRgb( r,g,b )
...
#         self.theCurrentImage = lPixmap.toImage()
    ...
    def paintTheImageNPContent(self, pThePainter):
        ...
        #
        if ( self.getAProcessingModeStatusForImageNP(CDConstants.ImageNPUseAreaCells) ) :
    ...
    def theTrueComputeCurrentEdge(self): 
        ...
        if ( self.getAProcessingModeStatusForImageNP(CDConstants.ImageNPUseDiscretizedToBWMode) == True ) :
            ...
            self.edgeInNPIsReadyFlags[self.theCurrentIndex] = False
    ...
    #         outpixels = list(outimg.getdata()) 
            ...
            for y in xrange(height): 
                ...
                for x in xrange(width): 
                    ...
                    if y == 0 or y == height-1: magnitude = 0 
                        ...
                        for i in xrange(-1, 2): 
                            for j in xrange(-1, 2): 
                                ...
                                #     approximating dI/dx 
                                r = int(self.volumeNPArray[self.theCurrentIndex, lY, lX, 0])
                                g = int(self.volumeNPArray[self.theCurrentIndex, lY, lX, 1])
                                b = int(self.volumeNPArray[self.theCurrentIndex, lY, lX, 2])
                                gray = (r + g + b) / 3
                    ...
                    self.edgeNPArray[self.theCurrentIndex, y, x, 0] = numpy.uint8 (255 - magnitude)
                    self.edgeNPArray[self.theCurrentIndex, y, x, 1] = numpy.uint8 (255 - magnitude)
                    self.edgeNPArray[self.theCurrentIndex, y, x, 2] = numpy.uint8 (255 - magnitude)
    
            # set the flag for the current edge array in the sequence as True, as we're computing it now:
            self.edgeInNPIsReadyFlags[self.theCurrentIndex] = True
    ...
    def normalizeAllImages(self):
        ...
        self.volumeNPArray = lTmpBoolArray.astype (numpy.uint8)

        lTheMaxVolumeArrayValue = self.volumeNPArray.max()
        ...
        if (lTheMaxVolumeArrayValue >= 1) and (lTheMaxVolumeArrayValue < 128):
            ...
            if (lTheMaxVolumeArrayValue < 2) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 7)
                CDConstants.printOut( " volumeNPArray 7 bit shift done " , CDConstants.DebugExcessive )
            elif (lTheMaxVolumeArrayValue < 4) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 6)
                CDConstants.printOut( " volumeNPArray 6 bit shift done " , CDConstants.DebugExcessive )
            elif (lTheMaxVolumeArrayValue < 8) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 5)
                CDConstants.printOut( " volumeNPArray 5 bit shift done " , CDConstants.DebugExcessive )
            elif (lTheMaxVolumeArrayValue < 16) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 4)
                CDConstants.printOut( " volumeNPArray 4 bit shift done " , CDConstants.DebugExcessive )
            elif (lTheMaxVolumeArrayValue < 32) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 3)
                CDConstants.printOut( " volumeNPArray 3 bit shift done " , CDConstants.DebugExcessive )
            elif (lTheMaxVolumeArrayValue < 64) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 2)
                CDConstants.printOut( " volumeNPArray 2 bit shift done " , CDConstants.DebugExcessive )
            else :   # i.e. (lTheMaxVolumeArrayValue < 128) :
                lTmpArray = numpy.left_shift( self.volumeNPArray, 1)
                CDConstants.printOut( " volumeNPArray 1 bit shift done " , CDConstants.DebugExcessive )

            self.volumeNPArray = lTmpArray
...
#                             "  lTmpBoolArray[k,j,i] = "+str(lTmpBoolArray[k, j, i])+"  self.volumeNPArray[k,j,i] = "+str(self.volumeNPArray[k,j,i]), CDConstants.DebugExcessive )
# 
...
#         print "--------------------------------------------------------------------------------"
# 
#         print "type(self.volumeNPArray) = ", type(self.volumeNPArray)
#         print "type(self.volumeNPArray[0,0,0,0]) = ", type(self.volumeNPArray[0,0,0,0]), " and contains ", self.volumeNPArray[0,0,0,0]
#         print "self.volumeNPArray.max() = ", self.volumeNPArray.max()
#         print "self.volumeNPArray.min() = ", self.volumeNPArray.min()
#         # print self.volumeNPArray
#         print "--------------------------------------------------------------------------------"
#         print "--------------------------------------------------------------------------------"
#         print "--------------------------------------------------------------------------------"
#         print "--------------------------------------------------------------------------------"
...
#                         lTmpPainter = QtGui.QPainter(lPixmap)
    ...
    def setCurrentIndexInImageNP(self, pValue):
        ...
        #  create images... theCurrentImage and theCurrentEdgeImage and theCurrentVolumeSliceImage from the current layer in the sequence arrays:
    ...
    def resetNPDimensions(self, pX, pY, pZ, pChannels):
        ...
        self.imageNPArray = numpy.zeros( \
            ...
            dtype=numpy.uint8  )
        self.edgeNPArray = numpy.zeros( \
            (self.sizeZ,  self.sizeY,  self.sizeX,  3), \
            dtype=numpy.uint8  )
        self.contoursNPArray = numpy.zeros( \
            (self.sizeZ,  self.sizeY,  self.sizeX,  3), \
            dtype=numpy.uint8  )
        self.volumeNPArray = numpy.zeros( \
            (self.sizeZ,  self.sizeY,  self.sizeX,  3), \
            dtype=numpy.uint8  )

        # reset to fresh and empty (all False) flag arrays as well:
        self.imageInNPIsReadyFlags = numpy.zeros( (self.sizeZ), dtype=numpy.bool )
        self.edgeInNPIsReadyFlags = numpy.zeros( (self.sizeZ), dtype=numpy.bool )
        self.contoursAreReadyFlag = False
...
#         self.processedPixmap = QtGui.QPixmap()
    ...
    def setScaleZoom(self, pValue):
        CDConstants.printOut( "___ - DEBUG ----- CDImageLayer.setScaleZoom( pValue == "+str(pValue)+") starting." , CDConstants.DebugExcessive )
    ...
    def setImage(self, pImage):
        ...
        if isinstance(pImage, QtGui.QImage):
            ...
            if self.imageLoadedFromFile == True:
                ...
                self.cdImageNP.setImage( QtGui.QImage(lResultPixmap.toImage()) )
...
#             self.floodFill(    pX, pY + 1, pOld, pNew)
    ...
    def floodFillFuzzyPick(self, pX, pY, pReplacementColor):
        CDConstants.printOut( "___ - DEBUG ----- CDImageLayer.floodFillFuzzyPick( pX, pY, pReplacementColor == "+str(pX)+" "+str(pY)+" "+strpReplacementColor+" ) starting." , CDConstants.DebugExcessive )
        lWidth = self.processedImage.width()
        lHeight = self.processedImage.height()
        ...
        # this is extract cells mode:
            if (self.myMouseLeftDown == False) and \
              (self._graphicsSceneWidget.view.hasMouseTracking() == True):
                ...
                # self.imageNPExtractCells(self.myMouseX, self.myMouseY)
                # self._graphicsSceneWidget.scene.update()
                pass
