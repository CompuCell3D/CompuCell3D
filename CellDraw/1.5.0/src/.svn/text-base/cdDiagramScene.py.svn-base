...
from cdControlRegionOrCell import ControlRegionOrCell
...
#         print "DiagramItem DIAGRAMITEM debugWhoIsTheRunningFunction() is", \
#           debugWhoIsTheRunningFunction(), \
#           "debugWhoIsTheParentFunction() is", \
#           debugWhoIsTheParentFunction()
        ...
        # print "type(pDiagramType).__name__ = ", type(pDiagramType).__name__
...
#         else (type(pDiagramType).__name__ is "QPainterPath"):
            print "DEBUG DEBUG ----- DiagramItem(): type(pDiagramType).__name__", \
                   type(pDiagramType).__name__
...
#                     QtCore.QPointF(-120, -80)])
            ...
            # print "self.thePathToBuildAPolygon =", self.thePathToBuildAPolygon
...
#         print "tmp polyg (diagramType=", self.diagramType,").boundingRect =",lTempGraphicsItem.polygon().boundingRect()
#         print "          hello, I'm %s, parent is %s" % (debugWhoIsTheRunningFunction(), debugWhoIsTheParentFunction())
#         print "          boundingRegionGranularity =", lTempGraphicsItem.boundingRegionGranularity()
...
#         print "          boundingRegion =", lBoundingRegion
#         print "          boundingRegion.boundingRect() =", lBoundingRegion.boundingRect()
#         print "          boundingRegion.rectCount() =", lBoundingRegion.rectCount()
...
#        
#         print "          lNewPainterPath =", lNewPainterPath
#         print "          lNewPainterPath.boundingRect() =", lNewPainterPath.boundingRect()
...
#        self.setPolygon(tmpPolygon)
        ...
        print "in DiagramItem._init_(): the bounding rect is =", lBoundingRect
        print "in DiagramItem._init_(): the rect's center is =", lCenterOfItemX, lCenterOfItemY
        ...
        for lPointF in self.myPolygon:
            ...
            # print "DiagramItem_init_(): lPointF, lZeroCenteredPointF =", lPointF, lZeroCenteredPointF
        ...
        print "          polygon (diagramType=", self.diagramType,").boundingRect =",self.polygon().boundingRect()
        print "          item (diagramType=", self.diagramType,").boundingRect =",self.boundingRect()
        print "          item (diagramType=", self.diagramType,").pen =",self.pen()
        print "          item (diagramType=", self.diagramType,").brush =",self.brush()
...
#         print "self.scene().myOutlineResizingItem =", self.scene().myOutlineResizingItem
...
#         # painter->drawRoundedRect(-10, -10, 20, 20, 5, 5);
    ...
    def setRegionOrCell(self, pRegionOrCell):
        ...
        if (self.itsaRegionOrCell == CDConstants.ItsaRegionConst):
            ...
            print "setRegionOrCell does CDConstants.ItsaRegionConst"
            ...
            print "setRegionOrCell does CDConstants.ItsaCellConst"
            ...
            print "setRegionOrCell does darkRed for UNKNOWN items"
        ...
        print "setRegionOrCell does item.pen().color().rgba() = ",self.pen().color().rgba()
        self.setPen(lMyPen)
        print "setRegionOrCell does item.pen().color().rgba() = ",self.pen().color().rgba()
    ...
    def setRegionID(self, pID):
        self.regionID = pID
        print "DiagramItem.setRegionID(",self.regionID,")"

    # ------------------------------------------------------------------
    def getRegionID(self):
        # print "DiagramItem.getRegionID() =",self.regionID
...
#            painter.translate(-110, 100)
    ...
    # ------------------------------------------------------------------
    def itemChange(self, change, value):
        # print "itemChange() itemChange() itemChange() itemChange() itemChange() itemChange() change, value =", change, value
...
class DiagramScene(QtGui.QGraphicsScene):
    ...
    def __init__(self, pEditMenu, pParent=None):
        ...
        if self.parentWidget.parentWindow is not None:
    ...
    def setImageLayer(self, pImageLayer = None):
        if isinstance( pImageLayer, CDImageLayer ) is True:
    ...
    def drawForeground(self, pPainter, pRect):
        ...
        if (self.mySceneMode == CDConstants.SceneModeImageLayer) and \
            (self.theImageLayer is not None):
            ...
            self.theImageLayer.paintEvent(pPainter)
        ...
        elif self.myOutlineResizingItem is not None:
...
#             lOutlinePen.setWidth(4)
            ...
            if self.myOutlineResizingVertex is "bottomLeft":
                pPainter.drawEllipse(lBoundingRect.bottomLeft(), (4 / self.myViewScaleZoomX), (4 / self.myViewScaleZoomY))
            elif self.myOutlineResizingVertex is "bottomRight":
                pPainter.drawEllipse(lBoundingRect.bottomRight(), (4 / self.myViewScaleZoomX), (4 / self.myViewScaleZoomY))
            elif self.myOutlineResizingVertex is "topRight":
                pPainter.drawEllipse(lBoundingRect.topRight(), (4 / self.myViewScaleZoomX), (4 / self.myViewScaleZoomY))
            elif self.myOutlineResizingVertex is "topLeft":
                pPainter.drawEllipse(lBoundingRect.topLeft(), (4 / self.myViewScaleZoomX), (4 / self.myViewScaleZoomY))
        ...
        elif self.myRightButtonResizingItem is not None:
    ...
    def addToRegionColorsInUse(self, pColor):
        ...
        # lColor is the color for which we have to update the usage count:
        ...
        for i in xrange(len(lRegionsTableDict)) :
            ...
            if lRegionsTableDict[lKeys[i]][0].rgba() == lColor.rgba() :
                ...
                if lColor.rgba() not in self.regionUseDict:
                    print "NO", lColor.rgba(), "not in", self.regionUseDict
                    # add a new entry to the self.regionUseDict local dictionary of used colors :
                    lCount = 1
                else:
                    print "YES", lColor.rgba(), "in", self.regionUseDict
                ...
                print self.regionUseDict[ lColor.rgba() ][0], self.regionUseDict[ lColor.rgba() ][1]
...
#             self.parentWidget.signalVisibilityPIFRegionTable.emit("Show")
        print "addToRegionColorsInUse - regionUseDict =", self.regionUseDict
        print "==================================== addToRegionColorsInUse"
    ...
    def subtractFromRegionColorsInUse(self, pColor):
        print "==================================== subtractFromRegionColorsInUse"
            ...
            if lRegionsTableDict[lKeys[i]][0].rgba() == lColor.rgba() :
                ...
                if lColor.rgba() not in self.regionUseDict:
                    ...
                    if (lCount < 1):
                        ...
                        print "self.regionUseDict has now no lColor.rgba() =", lColor.rgba()
                    else:
                        self.regionUseDict[ lColor.rgba() ] = [ lRegionsTableDict[lKeys[i]][1], lCount ]
                        print self.regionUseDict[ lColor.rgba() ][0], self.regionUseDict[ lColor.rgba() ][1]
...
#             #   "any empty sequence, for example, '', (), []" is considered false.
#             self.parentWidget.signalVisibilityPIFRegionTable.emit("Show")
        print "subtractFromRegionColorsInUse - regionUseDict =", self.regionUseDict
        print "==================================== subtractFromRegionColorsInUse"
    ...
    def setMode(self, mode):
        ...
        if (self.mySceneMode is CDConstants.SceneModeResizeItem) and \
            (mode is CDConstants.SceneModeResizeItem):
            pass
        else:
    ...
    def mousePressEvent(self, pMouseEvent, pThePIFItem=None):
        ...
        #   event processing, and pass the event to cdImageLayer's own
        ...
        if (self.mySceneMode == CDConstants.SceneModeImageLayer) and (pThePIFItem==None):
            if isinstance(self.theImageLayer, CDImageLayer) is True:
            ...
            # 2010 - Mitja: add code for handling outline resizing of items:
                ...
                if self.myOutlineResizingItem is not None:
                    ...
                    if self.myOutlineResizingVertex is not "None":
        ...
        #    (i.e. there is no mouse button pressed: we just called this function directly!)
            ...
            # print "pThePIFItem.brush().color().rgba() =", lRGBAValue
            if (lRGBAValue == 0):
        ...
        # 2010 - Mitja: implement resizing of items in real-time by right-clicking on them:
            ...
            if isinstance( lItemAt, QtGui.QGraphicsItem ):
                ...
                # print "mousePressEvent mousePressEvent mousePressEvent =", pMouseEvent
                # print "myRightButtonResizingItem myRightButtonResizingItem myRightButtonResizingItem =", self.myRightButtonResizingItem
                # print "lColor lColor lColor =", lColor
                # print "x, y =", self.myRightButtonResizingClickX, self.myRightButtonResizingClickY
            else:
                self.myRightButtonResizingItem = None
                self.myRightButtonResizingClickX = -1.0
                self.myRightButtonResizingClickY = -1.0
    ...
    def mouseMoveEvent(self, pMouseEvent):
        ...
        #   to resize them in real-time:
            ...
            # print "pMouseEvent.scenePos().x(), self.myRightButtonResizingClickX, sx, self.myRightButtonResizingItem.myScaleX =", pMouseEvent.scenePos().x(), self.myRightButtonResizingClickX, sx, self.myRightButtonResizingItem.myScaleX
            # print "pMouseEvent.scenePos().y(), self.myRightButtonResizingClickY, sy, self.myRightButtonResizingItem.myScaleY =", pMouseEvent.scenePos().y(), self.myRightButtonResizingClickY, sy, self.myRightButtonResizingItem.myScaleY
            # print "self.myRightButtonResizingItem.transformations() =", self.myRightButtonResizingItem.transformations()

        # 2010 - Mitja: implement a second outline for items, to resize them in real-time:
        elif self.mySceneMode == CDConstants.SceneModeResizeItem and self.myOutlineResizingItem is not None:
            ...
            # print " ------------------ lBoundingRect befor scaling =", lBoundingRect

            # A - compute scaling factors along X and Y according to mouse motion:
            if self.myOutlineResizingVertex is "topRight":
                ...
                sx = (bWidthX + dXmouse) / bWidthX
                sy = (bHeightY - dYmouse) / bHeightY
            elif self.myOutlineResizingVertex is "bottomLeft":
                sx = (bWidthX - dXmouse) / bWidthX
                sy = (bHeightY - dYmouse) / bHeightY
            else:
            ...
            # print "pMouseEvent.scenePos().x(), self.myOutlineResizingCurrentX, sx, self.myOutlineResizingItem.myScaleX =", pMouseEvent.scenePos().x(), self.myOutlineResizingCurrentX, sx, self.myOutlineResizingItem.myScaleX
            # print "pMouseEvent.scenePos().y(), self.myOutlineResizingCurrentY, sy, self.myOutlineResizingItem.myScaleY =", pMouseEvent.scenePos().y(), self.myOutlineResizingCurrentY, sy, self.myOutlineResizingItem.myScaleY
            # print "self.myOutlineResizingItem.transformations() =", self.myOutlineResizingItem.transformations()
    ...
    def mouseReleaseEvent(self, pMouseEvent):
        ...
        # 2010 - Mitja: implement right-clicking on items, to resize them in real-time:
            ...
            self.myRightButtonResizingClickX = -1.0
            self.myRightButtonResizingClickY = -1.0
            # print "mouseReleaseEvent ELSE mouseReleaseEvent ELSE mouseReleaseEvent =", pMouseEvent
            # print "myRightButtonResizingItem ELSE myRightButtonResizingItem ELSE myRightButtonResizingItem =", self.myRightButtonResizingItem
    ...
    def vvIsCloseDistance(self, pV1, pV2):
        ...
        x2 = pV2.x()
        y2 = pV2.y()
        # print "x1, y1,  x2, y2,  dist = ",  x1, y1,  x2, y2, math.sqrt( (x2-x1)*(x2-x1) + (y2-y1)*(y2-y1) )
...
class CDDiagramSceneMainWidget(QtGui.QWidget):
    ...
    def __init__(self, pParentWindow = None):
        ...
        self.theToggleforRegionOrCellDrawing = ControlRegionOrCell()
        ...
        #   to control QGraphicsScene's cdImageLayer's behavior, it's initially not enabled:
        ...
        # explicitly connect the "signalForPIFTableToggle()" signal from the
        #   theControlsForTypes object, to our "slot" method:
        answer = self.theControlsForTypes.signalForPIFTableToggle.connect( \
        ...
        self.cdImageLayer = CDImageLayer(self)
        self.scene.setImageLayer(self.cdImageLayer)
    ...
    def handlerForButtonGroupBackgroundsClicked(self, pButton):
        ...
        for myButton in theBgButtons:
            ...
            # print "handlerForButtonGroupBackgroundsClicked() myButton =",myButton,\
    ...
    # ------------------------------------------------------------
        ...
        self.scene.update()
        self.view.update()
        # print "___ - DEBUG ----- CDDiagramSceneMainWidget: updateSceneBackgroundImage() lBackgroundRect =", lBackgroundRect, "done."
...
#         print "handlePIFRegionTableButton"
...
#         print "handlePIFRegionTableButton"
    ...
    def copyItem(self, pItemHasBeenCut = False):
        ...
        if not lAllSelectedItems:
            # print "#=#=#=#=#=#=#=# copyItem() lAllSelectedItems =", lAllSelectedItems
            return
        ...
        for lPointF in lSelectedItemPolygon:
            # print "lPointF = ", lPointF
            lPointListX.append(lPointF.x())
            lPointListY.append(lPointF.y())

        # print "COPY: lSelectedItemPosition =", lSelectedItemPosition
        # print "COPY: lSelectedItemHasBeenCut =", lSelectedItemHasBeenCut
        # print "COPY: lSelectedItemColor =", lSelectedItemColor
        # print "COPY: lSelectedItemRegionOrCell = lSelectedItemRegionOrCell
        # print "COPY: lSelectedItemType =", lSelectedItemType
        # print "COPY: lSelectedItemScaleX =", lSelectedItemScaleX
        # print "COPY: lSelectedItemScaleY =", lSelectedItemScaleY
        # print "COPY: lPointListX =", lPointListX
        # print "COPY: lPointListY =", lPointListY
    ...
    def pasteItem(self):
        ...
        if lMimeData.hasFormat('application/x-pif-region-item'):
            ...
            print "PASTE: lListOfPointsX =", lListOfPointsX
            print "PASTE: lListOfPointsY =", lListOfPointsY
            lLengthX = len(lListOfPointsX)
            lLengthY = len(lListOfPointsY)
            if lLengthX == lLengthY:
                pass
            else:
                print "PASTE: received INCONSISTENT paste data! Can't paste."
                return


            lSelectedItemPolygon = QtGui.QPolygonF()
            # print "PASTE: lSelectedItemPolygon =", lSelectedItemPolygon


            for i in xrange(lLengthX):
                # print "PASTE: lListOfPointsX[i].toFloat() = ", lListOfPointsX[i].toFloat()
                # print "PASTE: lListOfPointsY[i].toFloat() = ", lListOfPointsX[i].toFloat()
                lPointF = QtCore.QPointF(lListOfPointsX[i].toFloat()[0], lListOfPointsY[i].toFloat()[0])
                # print "PASTE: lPointF =", lPointF
                lSelectedItemPolygon.append(lPointF)


            print "PASTE: lSelectedItemPosition =", lSelectedItemPosition
            print "PASTE: lSelectedItemHasBeenCut =", lSelectedItemHasBeenCut
            print "PASTE: lSelectedItemColor =", lSelectedItemColor
            print "PASTE: lSelectedItemRegionOrCell =", lSelectedItemRegionOrCell
            print "PASTE: lSelectedItemType =", lSelectedItemType
            print "PASTE: lSelectedItemScaleX =", lSelectedItemScaleX
            print "PASTE: lSelectedItemScaleY =", lSelectedItemScaleY
            print "PASTE: lSelectedItemPolygon =", lSelectedItemPolygon
            ...
            if lSelectedItemHasBeenCut is False:
    ...
    def handleLayersSelectionModeHasChanged(self, pNewMode):
        # print "handleLayersSelectionModeHasChanged(pNewMode=",pNewMode,"), "
        ...
        if self.scene.getMode() == CDConstants.SceneModeImageLayer:
            # 2011 - Mitja: enable SceneModeImageLayer-specific buttons:
    ...
    def handlerForItemInserted(self, item):
        ...
        try:
            ...
            print "EXCEPTION EXCEPTION EXCEPTION item item item item =", item
            pass
...
#         self.theControlsForInputImagePicking.setEnabled(False)
    ...
    def handlerForSceneResized(self, pDict):
        lDict = dict(pDict)
        print
        print "    TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO "
        print lDict
        print "    TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO "
        print
        self.windowPIFControlPanel.setSceneWidthLabel(lDict[0])
        self.windowPIFControlPanel.setSceneHeightLabel(lDict[1])
        self.windowPIFControlPanel.setSceneUnitsLabel(lDict[2])
    ...
    def handleImageScaleZoomChanged(self, pScale):
        ...
        if isinstance(self.scene.theImageLayer, CDImageLayer) is True:
            self.scene.theImageLayer.setScaleZoom( lNewScale )

        self.scene.update()

        print "      self.cdImageLayer.scaleFactor =", \
          self.cdImageLayer.scaleFactor
    ...
    def handleTypesColorEvent(self, pColor):
        # print "handleTypesColorEvent received pColor =", pColor
    ...
    def handleToggleRegionOrCellDrawingChanged(self, pRegionOrCell):
        ...
        print "      self.handleToggleRegionOrCellDrawingChanged:", pRegionOrCell
    ...
    def handleInputImagePickingModeChanged(self):
        ...
        self.cdImageLayer.inputImagePickingMode = \
        ...
        if (self.cdImageLayer.inputImagePickingMode == CDConstants.ImageModeDrawPolygon):
            self.cdImageLayer.setMouseTracking(True)
        elif (self.cdImageLayer.inputImagePickingMode == CDConstants.ImageModePickColor):
            self.cdImageLayer.setMouseTracking(True)
        else:
            self.cdImageLayer.setMouseTracking(False)

        self.scene.update()

        print "      self.cdImageLayer.inputImagePickingMode =", \
          self.cdImageLayer.inputImagePickingMode
    ...
    def handleImageOpacityChanged(self):
        ...
        print "      self.cdImageLayer.imageOpacity =", \
          self.cdImageLayer.imageOpacity
    ...
    def handleFuzzyPickThresholdChanged(self):
        ...
        print "      self.cdImageLayer.fuzzyPickTreshold =", \
          self.cdImageLayer.fuzzyPickTreshold



    # ---------------------------------------------------------
    # 2011 - Mitja: pickColorRegionFromInputImage() is called every time
    #   the cdImageLayer object emits a "mousePressedInImageLayerSignal()" signal,
    #   (defined in the CDImageLayer class's mousePressEvent() function),
    # ---------------------------------------------------------
    def pickColorRegionFromInputImage(self):

        # 2011 - Mitja: only do something about mouse clicks within the cdImageLayer
        #   if the cdImageLayer contains an image loaded from a file.
        # Otherwise do nothing:
        if self.cdImageLayer.imageLoadedFromFile is False:
            print "2011 DEBUG: ---- ---- CDDiagramSceneMainWidget ---- ---- pickColorRegionFromInputImage() has no input image. Returning."
            return

        # 2011 - Mitja: obtain the x,y coordinates where the mouse was clicked
        #   within the cdImageLayer:
        x = self.cdImageLayer.myMouseX
        y = self.cdImageLayer.myMouseY
        ...
        lRGBAColorAtClickedPixel = self.cdImageLayer.processedImage.pixel(x,y)
        ...
        if  (self.parentWindow.ignoreWhiteRegionsForPIF == True) and (lRGBAColorAtClickedPixel == QtGui.QColor(QtCore.Qt.white).rgba()) :
            print "2011 DEBUG: ---- ---- CDDiagramSceneMainWidget ---- ---- pickColorRegionFromInputImage() clicked on white. Returning."
            return # do nothing
        if (self.parentWindow.ignoreBlackRegionsForPIF == True) and (lRGBAColorAtClickedPixel == QtGui.QColor(QtCore.Qt.black).rgba()) :
            print "2011 DEBUG: ---- ---- CDDiagramSceneMainWidget ---- ---- pickColorRegionFromInputImage() clicked on black. Returning."
            return # do nothing
        ...
        if lRGBAColorAtClickedPixel in self.parentWindow.colorDict:
            ...
            for lRegionColor in self.parentWindow.colorIds:
                r1 = QColor(lRGBAColorAtClickedPixel).redF()
                g1 = QColor(lRGBAColorAtClickedPixel).greenF()
                b1 = QColor(lRGBAColorAtClickedPixel).blueF()
                r2 = QColor(lRegionColor).redF()
                g2 = QColor(lRegionColor).greenF()
                b2 = QColor(lRegionColor).blueF()
                d =   ((r2-r1)*0.30) * ((r2-r1)*0.30) \
                    + ((g2-g1)*0.59) * ((g2-g1)*0.59) \
                    + ((b2-b1)*0.11) * ((b2-b1)*0.11)
                print "r1, g1, b1, r2, g2, b2, d, lColorDistance, lRGBAClosestColor =", r1, g1, b1, r2, g2, b2, d, lColorDistance, lRGBAClosestColor
                if (lColorDistance > d) :
                    lRGBAClosestColor = lRegionColor
                    lColorDistance = d

        print "Inserting"
        print lRGBAColorAtClickedPixel
        print lRGBAClosestColor
        print self.parentWindow.comboDict[lRGBAClosestColor]
        ...
        if (self.parentWindow.pickColorRegion == True) :
            ...
            # 1. get a *pixmap copy* of the QImage from cdImageLayer:
            lOriginalPixmap = QtGui.QPixmap( QtGui.QPixmap.fromImage(self.cdImageLayer.processedImage) )
            # 2. create a "mask" QBitmap object from lOriginalPixmap's pixels NOT in the clicked color:
            lTheMaskBitmap = lOriginalPixmap.createMaskFromColor(lClickedPixelColor,  Qt.MaskOutColor)
            # 3. apply lTheMaskBitmap to lOriginalPixmap, so that it eliminates all other color pixels from it:
            # ------> BUT the setMask() function applied to lOriginalPixmap will ALSO mask the cdImageLayer... WHY???
            # lOriginalPixmap.setMask( lTheMaskBitmap )

            # 3. create an empty pixmap where to store the composed image:
            lResultPixmap = QtGui.QPixmap(self.cdImageLayer.width, self.cdImageLayer.height)
            # 4. create a QPainter to perform the overlay operation:
            lPainter = QtGui.QPainter(lResultPixmap)
            # 5. do the overlay:
            lPainter.setCompositionMode(lPainter.CompositionMode_Source)
            ...
            # don't copy the input image back to cdImageLayer:
            # self.cdImageLayer.setPixmap(lResultPixmap)
            # self.scene.update()
            ...
            self.cdImageLayer.setToProcessedImage()
...
#                 self.loadScenePIFDataFromFile(fileName)
    ...
    def loadScenePIFDataFromFile(self, pFileName):
        ...
        for i in xrange(lHowManyItems) :
            ...
            lListOfPointsX = lDataStream.readQVariant().toList()
            lListOfPointsY = lDataStream.readQVariant().toList()
#             print "LOAD: lListOfPointsX =", lListOfPointsX
#             print "LOAD: lListOfPointsY =", lListOfPointsY
            lLengthX = len(lListOfPointsX)
            lLengthY = len(lListOfPointsY)
            if lLengthX == lLengthY:
                pass
            else:
                print "LOAD: received INCONSISTENT paste data! Can't paste."
                return


            lSelectedItemPolygon = QtGui.QPolygonF()
            # print "LOAD: lSelectedItemPolygon =", lSelectedItemPolygon


            for i in xrange(lLengthX):
                # print "LOAD: lListOfPointsX[i].toFloat() = ", lListOfPointsX[i].toFloat()
                # print "LOAD: lListOfPointsY[i].toFloat() = ", lListOfPointsX[i].toFloat()
                lPointF = QtCore.QPointF(lListOfPointsX[i].toFloat()[0], lListOfPointsY[i].toFloat()[0])
                # print "LOAD: lPointF =", lPointF
                lSelectedItemPolygon.append(lPointF)


#             print "LOAD: lSelectedItemPosition =", lSelectedItemPosition
#             print "LOAD: lSelectedItemColor =", lSelectedItemColor
#             print "LOAD: lSelectedItemRegionOrCell =", lSelectedItemRegionOrCell
#             print "LOAD: lSelectedItemType =", lSelectedItemType
#             print "LOAD: lSelectedItemScaleX =", lSelectedItemScaleX
#             print "LOAD: lSelectedItemScaleY =", lSelectedItemScaleY
#             print "LOAD: lSelectedItemZValue =", lSelectedItemZValue
#             print "LOAD: lSelectedItemPolygon =", lSelectedItemPolygon
...
#         return True
    ...
    def saveScenePIFDataToFile(self, pFileName):
        ...
        print lHowManyItems, lAllItems

        # make sure that there is at least one item in the scene:
        if not lAllItems:
            print "#=#=#=#=#=#=#=# saveScenePIFDataToFile() lAllItems =", lAllItems
            print
            print "there is NOTHING in your Cell Scene!!!"
            print
            return
    ...
    def about(self):
        ...
        lAboutString = "CellDraw 1.5.0<br><br>An editing and conversion software tool for PIFF files, as used in CompuCell3D simulations.<br><br>CellDraw can be useful for creating PIFF files containing a high number of cells and cell types, either by drawing a scene containing cell regions in a paint program, and then discretize the drawing into a PIFF file, or by drawing the cell scenario directly in CellDraw.<br><br>More information at:<br><a href=\"http://www.compucell3d.org/\">http://www.compucell3d.org/</a>"
    ...
    def connectSignals(self):
        ...
        # 2011 - Mitja: connect the sender (i.e. self.cdImageLayer) object's signal
        #  (i.e. "mousePressedInImageLayerSignal()" in CDImageLayer's mousePressEvent() ),
        #  to the receiver fuction (i.e. pickColorRegionFromInputImage() here.) :
        self.connect( self.cdImageLayer,  \
            QtCore.SIGNAL("mousePressedInImageLayerSignal()"), \
            self.pickColorRegionFromInputImage )
    ...
    def createSceneEditActions(self):
        ...
        self.cutAction = QtGui.QAction(
                ...
                shortcut=QtGui.QKeySequence.Cut, statusTip="Cut a region from the Cell Scene",
                triggered=self.cutItem)

        self.copyAction = QtGui.QAction(
                QtGui.QIcon(':/icons/copyRegion.png'), "Copy", self,
                shortcut=QtGui.QKeySequence.Copy, statusTip="Copy a region from the Cell Scene",
                triggered=self.copyItem)

        self.pasteAction = QtGui.QAction(
                QtGui.QIcon(':/icons/pasteRegion.png'), "Paste", self,
                shortcut=QtGui.QKeySequence.Paste, statusTip="Paste a region to the Cell Scene",
        ...
        print "___ - DEBUG ----- CDDiagramSceneMainWidget: createSceneEditActions() done."
    ...
    def updateSceneRectSize(self):
        ...
        for myButton in buttons:
            if myButton.isChecked() is True:
                lText = myButton.text()
                # print "myButton.text() =", myButton.text()
