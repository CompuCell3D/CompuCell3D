...
#         self.processedPixmap = QtGui.QPixmap()
    ...
    def setImageLoadedFromFile(self, pTrueOrFalse):
        ...
        if isinstance( self.graphicsSceneWindow, QtGui.QWidget ) is True:
            self.graphicsSceneWindow.scene.update()
...
#             self.processedPixmap = QtGui.QPixmap(pPixmap)
    ...
    def setImage(self, pImage):
        if isinstance(pImage, QtGui.QImage):
            ...
            if self.imageLoadedFromFile is True:       
            ...
            for (pX, pY) in edge:
                for (s, t) in ((pX+1, pY), (pX-1, pY), (pX, pY+1), (pX, pY-1)):
                    if (s >= 0) and (s < lWidth) and (t >= 0) and (t < lHeight) :
                        ...
                        if (self.colorToColorIsSame( pReplacementColor, lColorAtST) is not True) and \
...
#
    ...
    # ------------------------------------------------------------------
    # 2011 - Mitja: this function draws into the passed QPainter parameter
    # ------------------------------------------------------------------   
    def paintEvent(self, pThePainter):
        ...
        # print "|||||", self.repaintEventsCounter, \
        #       "||||| CDImageLayer.pThePainter==", pThePainter

        # 2011 - Mitja: this function is called to draw into the QPainter parameter:
        lPainter = pThePainter

        # draw the input image, if there is one:
        if isinstance( self.processedImage, QtGui.QImage ) is True:
...
#             lPainter.setOpacity(lTmpOpacity)
        ...
        # this is color-picking mode:
            ...
            if (self.myMouseLeftDown == True):
                ...
                lPen = QtGui.QPen()
                if self.myPathHighlight is True:
...
#         print "2010 DEBUG:", self.repaintEventsCounter, "CDImageLayer.drawGrid() DONE - WWIIDDTTHH(image,pifScene) =", self.width,self.pifSceneWidth, "HHEEIIGGHHTT(image,pifScene) =", self.height,self.pifSceneHeight
    ...
    def mousePressEvent(self, pEvent):       
        ...
        if pEvent.button() == QtCore.Qt.LeftButton:
            # print "CLICK inside CDImageLayer"
            ...
            if (self.inputImagePickingMode == CDConstants.ImageModePickColor):
            # this is color-picking mode:
                color = self.theImage.pixel(pEvent.scenePos().x(),pEvent.scenePos().y())
            ...
            # print"2010 DEBUG: CDImageLayer.mousePressEvent() - pEvent(x,y) =",pEvent.scenePos().x(),pEvent.scenePos().y()
    ...
    def mouseReleaseEvent(self, pEvent):
        ...
        # this is freeform drawing mode:
            ...
            if (pEvent.button() == QtCore.Qt.LeftButton):
                ...
                if len(self.theCurrentPath) >= 3:
                    ...
                    lXa = -1
                    lYa = -1
                    for lX, lY in self.theCurrentPath:
                        if lXa == -1:
                            lXa = lX
                            lYa = lY
                            thePainterPath.moveTo(lX,lY)
                        else:
                            thePainterPath.lineTo(lX,lY)
        ...
        # print"2010 DEBUG: CDImageLayer.mouseReleaseEvent() - pEvent(x,y) =",pEvent.scenePos().x(),pEvent.scenePos().y()
        ...
        # this is freeform drawing mode:
            if (self.myMouseLeftDown == True) and \
              (self.graphicsSceneWindow.view.hasMouseTracking() == False):
                ...
                lX, lY = self.theCurrentPath[0]
                if (abs(self.myMouseX - lX) < 3) and \
                   (abs(self.myMouseY - lY) < 3):
